...
These variables apply when the container **entrypoint** builds a ``cuopt_grpc_server`` command (see `Docker: gRPC Server in Container <docker-grpc-server-in-container>`_ under Usage). If you pass an explicit command after the image name, this table does not apply.
...
.. _docker-grpc-server-in-container:
...
2. **`CUOPT_SERVER_TYPE=grpc`** builds a ``cuopt_grpc_server`` command from the **NVIDIA cuOpt container** table in `Configuration Parameters <configuration-parameters>`_.
...
**Required** for remote (see `Bundled Remote Client <bundled-remote-client>`_ table for all variables):
